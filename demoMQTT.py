...
def main():
    ...
    pi = pigpio.pi()
    device = 'Enc1'  # Device name should be unique, can not duplicate device ID
    lvl2 = 'encoder' # Topic lvl2 name can be a duplicate, meaning multiple devices publishing data on the same topic
    publvl3 = MQTT_CLIENT_ID + "" # Will be a tag in influxdb. Optional to modify it and describe experiment being ran
    data_keys = ['rpm1i'] # If topic lvl2 name repeats would likely want the data_keys to be unique
    dtPin = 5
    setup_device(device, lvl2, publvl3, data_keys)
    EncoderSet[device] =  Encoder(pi, dtPin, *data_keys, pulses_per_rev=20.0, min_RPM=5.0, logger=logger_encoder)
    ...
    t0_sec = perf_counter() # sec Counter for getting data. Future feature - update interval in  node-red dashboard to link to perf_counter
    ...
    try:
        while True:
            ...
            if (perf_counter() - t0_sec) > msginterval: # getdata() from devices on msginterval (also publish data). Note - Does not affect on_message/mqtt data. on_message runs in parallel
                t0_sec = perf_counter()
                for device, encoder in EncoderSet.items(): # ** Remove rotary encoder from msginterval loop for real application
                    deviceD[device]['data'] = encoder.getdata()
        ...
        for device, encoder in EncoderSet.items():
            encoder.cancel()
        pi.stop()
        main_logger.info(f"{pcolor.CYAN}pigpio stopped and encoder cancelled{pcolor.ENDC}")
